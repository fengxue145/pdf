...
class Tag
{
	 ...
	 * @var array
	 */
	private static $custome_tags_map = [
		'BARCODE' => 'BarCode',
	...
	/**
	 * copy by \Mpdf\Tag::getTagInstance()
	 * 
	...
	/**
	 * Set the fully qualified name of the class handling the rendering of the given tag
	 * 
	 * @param string $tag The tag name
	 * @param string $className The fully qualified name
	 */
	public static function setTagClassName($tag, $className)
	{
		self::$custome_tags_map[strtoupper($tag)] = $className;
	}
	...
	{
		$className = isset(self::$custome_tags_map[$tag]) ? self::$custome_tags_map[$tag] : ucfirst(strtolower($tag));

		if (substr($className, 0, 1) !== '\\') {
			if (class_exists(__NAMESPACE__ . '\Tag\\' . $className)) {
				return __NAMESPACE__ . '\Tag\\' . $className;
			} elseif (class_exists('\Mpdf\Tag\\' . $className)) {
				return '\Mpdf\Tag\\' . $className;
			}
		}
		return $className;
	}
	...
	public function OpenTag($tag, $attr, &$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->allow_html_optional_endtags) {
			if (isset($this->mpdf->blk[$this->mpdf->blklvl]['tag'])) {
				...
				if (
					!$closed && $this->mpdf->blk[$this->mpdf->blklvl]['tag'] == 'P'
